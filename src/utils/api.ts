...
const formatErrorDetail = (detail: string | ValidationError[] | undefined): string => {
...
const handleApiError = (error: AxiosError, showError: boolean, show404Error: boolean = true) => {
  ...
  if (errorMessage.status === 401 && errorMessage.message.includes('login')) {
    showToast('Session expired. Please login again.', 'error');
    window.location.href = '/';
  } else if (
    ((errorMessage.statusCode && errorMessage.statusCode >= 404) || errorMessage.status >= 404) &&
    show404Error
  ) {
